...
export class RatingsService {
  ...
  async create(createRatingDto: CreateRatingDto) {
    try {
      ...
      const userRating = await this.userModel.findById(
        createRatingDto.ratee_id,
      );
      let total_ratings = 1;
      ...
      if (userRating && userRating['reputation']) {
        ...
        total_score =
          (userRating['reputation'].total_score || 0) + createRatingDto.score;
      }
      this.userModel
        .findByIdAndUpdate(createRatingDto.ratee_id, {
          reputation: {
      ...
      this.logger.error('Error creating rating', error.stack);
      ...
      try {
        ...
        if (!userRating || !userRating.reputation) {
          throw new NotFoundException(
            'Không tìm thấy người dùng hoặc danh tiếng để cập nhật',
          );
        }

        await this.userModel
          .findByIdAndUpdate(oldRating.ratee_id, {
            $set: {
              'reputation.total_score':
                (userRating.reputation.total_score || 0) -
                oldScore +
                (updateRatingDto.score || 0),
            },
          })
          .exec();
        ...
        this.logger.error('Error updating rating', error.stack);
        throw new Error('Không thể cập nhật đánh giá');
      }
    }
  }

  async findOneByRaterAndRatee(
    rater_id: string,
    ratee_id: string,
  ): Promise<Rating> {
    if (
      !Types.ObjectId.isValid(rater_id) ||
      !Types.ObjectId.isValid(ratee_id)
    ) {
    ...
    if (!rating) {
      throw new NotFoundException(
        'Không tìm thấy đánh giá cho cặp người dùng này',
      );
    }
