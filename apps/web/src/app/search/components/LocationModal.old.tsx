...
interface LocationModalProps {
  ...
  onProvinceCodeChange?: (code: number | null) => void; // ✅ Callback khi province code thay đổi
  setSelectedWardName?: (name: string) => void;
  onWardCodeChange?: (code: number | null) => void; // ✅ Callback khi ward code thay đổi
...
export default function LocationModal({
  ...
  onProvinceCodeChange, // ✅ Callback khi province code thay đổi
  setSelectedWardName,
  onWardCodeChange, // ✅ Callback khi ward code thay đổi
  initialProvinceCode, // ✅ Province code từ URL (khi refresh)
}: LocationModalProps) {
  // States for provinces API
  const [provinces, setProvinces] = useState<Province[]>([]);
  const [wards, setWards] = useState<Ward[]>([]);
  const [localProvinceCode, setLocalProvinceCode] = useState<number>(
    initialProvinceCode || 0 // ✅ Sử dụng initialProvinceCode từ URL
  );
  const [localWardCode, setLocalWardCode] = useState<number>(0);
    ...
    async function fetchWards() {
      try {
        ...
        const res = await fetch(
          `/api/wards?provinceCode=${localProvinceCode}&v2=true`
        );
        if (!mounted) return;
        const data = await res.json();
        setWards(data || []);
        // Set default to first ward if not already set
        if (data && data.length > 0 && !localWardCode) {
          setLocalWardCode(data[0].code);
        }
      } catch (error) {
  ...
  const detectCurrentLocation = async (provincesList: Province[]) => {
    if (!navigator.geolocation) {
      ...
      if (provincesList.length > 0) {
        setLocalProvinceCode(provincesList[0].code);
      }
      return;
    }
      ...
      async (position) => {
        try {
          ...
          if (detectedProvinceName) {
            ...
            if (matchedProvince) {
              setLocalProvinceCode(matchedProvince.code);
              ...
              if (detectedWard) {
                ...
                try {
                  ...
                  if (matchedWard) {
                    setLocalWardCode(matchedWard.code);
                    setSelectedLocation(
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.section}>
            ...
            {(localProvinceCode > 0 || localWardCode > 0) && (
              <div
                style={{
                  display: "flex",
                  gap: "8px",
              ...
              >
                {localProvinceCode > 0 && (
                  ...
                  >
                    ...
                    <span>
                      {provinces.find((p) => p.code === localProvinceCode)
                        ?.name || "Tất cả tỉnh"}
                    </span>
                  </div>
                )}
                {localWardCode > 0 && (
                  ...
                  >
                    ...
                    <span>
                      {wards.find((w) => w.code === localWardCode)?.name ||
                        "Tất cả phường"}
                    </span>
                  </div>
                )}
            ...
            <div className={styles.locationSelects}>
              ...
              <select
                ...
                disabled={
                  loading || wards.length === 0 || localProvinceCode === 0
                }
              >
        ...
        <div className={styles.modalFooter}>
          <button
            ...
            onClick={() => {
              ...
              if (provinces.length > 0) {
                setLocalProvinceCode(0); // Set to "Tất cả"
              }
              setLocalWardCode(0);
          ...
          <button
            ...
            onClick={() => {
              ...
              const ward = wards.find((w) => w.code === localWardCode);

              if (province && ward) {
                setSelectedLocation(`${ward.name}, ${province.name}`);
                // ✅ Set province name + code
                if (setSelectedProvinceName) setSelectedProvinceName(province.name);
                if (onProvinceCodeChange) onProvinceCodeChange(province.code);
                // ✅ Set ward name + code
                if (setSelectedWardName) setSelectedWardName(ward.name);
                if (onWardCodeChange) onWardCodeChange(ward.code);
              } else if (province) {
                ...
                if (onProvinceCodeChange) onProvinceCodeChange(province.code);
                // ✅ Clear ward
                if (setSelectedWardName) setSelectedWardName("");
                if (onWardCodeChange) onWardCodeChange(null);
              }
              onClose();
            }}
            disabled={loading}
