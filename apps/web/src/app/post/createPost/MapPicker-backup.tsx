...
export default function MapPicker({
  ...
  function cleanDisplayName(s: string) {
    // Đảm bảo hàm an toàn khi sử dụng trên UI: luôn trả về một chuỗi.
    // - null/undefined -> chuỗi rỗng (an toàn hơn cho giá trị đầu vào)
    // - các giá trị không phải chuỗi khác -> ép buộc thành chuỗi
    if (s == null) return "";
    if (typeof s !== "string") s = String(s);
    let out = s;
    // xóa mã bưu chính (4-6 chữ số) đứng riêng lẻ hoặc có dấu phẩy/khoảng cách đứng trước/sau;
    out = out.replace(/\b\d{4,6}\b/g, "");
    // remove country suffixes like ', Việt Nam' or ', Vietnam'
    out = out.replace(/,?\s*(Việt Nam|Vietnam)\s*$/iu, "");
    // collapse repeated commas and whitespace
